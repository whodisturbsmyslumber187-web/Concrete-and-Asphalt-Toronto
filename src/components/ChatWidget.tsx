...
const ChatWidget = () => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([
        {
          role: "assistant",
          content: "Hello! How can I help you with your concrete or asphalt project today?",
        },
      ]);
    }
  }, [isOpen]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Sorry, something went wrong. Please try again or give us a call at (416) 555-1234.",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const openWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      `Hi, I'm interested in your concrete/asphalt services. ${lastUserMessage}`
    );
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gold text-primary rounded-full shadow-luxury flex items-center justify-center hover:bg-gold-dark transition-colors"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-48px)] bg-card rounded-lg shadow-luxury overflow-hidden border border-border"
          >
            {/* Header */}
